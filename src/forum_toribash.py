...
class ForumToribash:
	...
	def login(self, username: str, password: str) -> dict:
		data = {
			...
			"vb_login_md5password": self.md5_hash(password),
			"vb_login_md5password_utf": self.md5_hash(password),
			"cookieuser": 1,
			"do": "login",
			"securitytoken": "guest",
			"format": "json"
	...
	def get_market_item(self, item_id: int) -> dict:
		return convert(
			requests.get(
	...
	def get_toribash_staff(self) -> dict:
		return convert(
			requests.get(
